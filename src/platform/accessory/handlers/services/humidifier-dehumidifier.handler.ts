...
> {
  ...
  > = {
      ...
      controlLock: [
        {
          ...
          onSet: (value) => value === Characteristic.LockPhysicalControls.CONTROL_LOCK_ENABLED,
        },
      ],
      mode: [],
      mistLevel: [],
      ...
      targetHumidity: [ {
        ...
        configure: (value) => {
          const rangeValue = value as unknown as { range?: { min?: number, max?: number; }; };
          if (rangeValue === undefined || rangeValue.range?.min === undefined || rangeValue.range?.max === undefined) {
            return {};
          }
          return {
