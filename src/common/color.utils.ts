...
export const rgbToKelvin = (rgbColor: ColorRGB): number =>
  parseInt(
    Object.entries(KELVIN_MAP)
      .map((entry) => {
        return {
          temperature: entry[0],
          color: entry[1],
...
export const kelvinToRGB = (kelvin: number): ColorRGB =>
  KELVIN_MAP[Math.max(Math.min(Math.round(kelvin / 100) * 100, 7100), 2000)];

export const rgbToHSV = (rgb: ColorRGB): HueSaturation => {
  const hsv = rgb2hsv([rgb.red, rgb.green, rgb.blue]);
  return new HueSaturation(hsv[0], hsv[1]);
};

export const hsvToRGB = (hue: number, saturation: number): ColorRGB => {
